...
\thanks{Artefatos disponíveis em:
...
% ─── ABSTRACT (English) ──────────────────────────────────────────────────────
\begin{abstract}
Critical infrastructures --- particularly electric distribution networks
...
\begin{IEEEkeywords}
RPL, FAN, \textit{backhaul} sem fio, LLN, DODAG, \textit{smart grid}, automação de distribuição,
...
Três tecnologias dominam o \textit{backhaul} em infraestruturas críticas hoje:
...
Redes de campo sem fio --- denominadas \textit{Field Area Networks} (FANs) ou
\textit{Neighborhood Area Networks} (NANs) no contexto de medição avançada --- operam na
camada entre os dispositivos de campo (medidores, sensores, atuadores) e os
concentradores de dados ou \textit{gateways} que se conectam à rede de gestão. A
...
conectam ao sistema de \textit{back-office} via WAN~\cite{ancillotti2013rpl}.

\begin{figure}[t]
  \centering
  \begin{tikzpicture}[
    dev/.style   = {circle, draw=black!55, fill=gray!25,
                    minimum size=5pt, inner sep=0},
    infra/.style = {rectangle, rounded corners=2pt,
    ...
    \node[dev] (d2) at (0.0,  0.8) {};
    \node[dev] (d3) at (0.6,  1.3) {};
    \node[dev] (d4) at (0.6,  0.4) {};
    \node[dev] (d5) at (0.3, -0.5) {};
    \node[dev] (d6) at (1.3,  0.8) {};

    %% ── infrastructure ──────────────────────────────────────
    \node[infra] (dap) at (2.6, 0.4) {Concentrador\\(DAP)};
    \node[infra] (he)  at (4.6, 0.4) {Head-end\\SCADA};
    ...
    %% ── WAN arrow (label as separate node above) ────────────
    \draw[wan] (dap.east) -- (he.west);
    \node[font=\tiny, align=center, text=black!65]
      at (3.6, 1.0) {LTE Band~31\\(450\,MHz)};

    %% ── FAN zone background ─────────────────────────────────
    \begin{scope}[on background layer]
      \fill[gray!10, rounded corners=4pt]
        (-0.4,-0.85) rectangle (2.05, 1.7);
    \end{scope}

    %% ── zone labels ─────────────────────────────────────────
    \node[font=\tiny, align=center, text=black!55]
      at (0.8, -1.2) {FAN --- RPL / IEEE~802.15.4g / Wi-SUN};
    \node[font=\tiny, text=black!55]
      at (3.6, -1.2) {WAN};

    %% ── device legend ───────────────────────────────────────
    \node[dev, scale=0.65] at (-0.4, 2.0) {};
    \node[font=\tiny, text=black!60, right]
      at (-0.2, 2.0) {Medidor / Sensor / Atuador};
  \end{tikzpicture}
...
de rotas, suporta topologias com milhares de nós e opera com \textit{overhead} de
controle compatível com dispositivos embarcados~\cite{ancillotti2013rpl}.

Este trabalho apresenta o RPL no contexto de \textit{backhaul} FAN, organizado da
...
O RPL utiliza o algoritmo \textit{Trickle}~\cite{rfc6206} para controlar a frequência de
transmissão dos DIOs. O \textit{Trickle} adapta o intervalo entre limites mínimo
...
preferencial por múltiplos intervalos \textit{Trickle} indica falha do nó pai.
...
algoritmo \textit{Trickle} reiniciado a $I_{min}$ propaga o novo DIO rapidamente pela
...
RPL em \textit{backhaul} de campo. Medidores inteligentes, instalados em transformadores
...
roteamento completas nos medidores, e o algoritmo \textit{Trickle} garante que o overhead
...
O \textit{backhaul} sem fio via RPL substitui com vantagem econômica os enlaces ponto-a-
...
nos mecanismos de segurança e nos parâmetros \textit{Trickle}.

% ─── VI. CONCLUSÃO ───────────────────────────────────────────────────────────
\section{Conclusão}
\label{sec:conclusion}

O RPL consolida-se como o protocolo de roteamento padrão para \textit{backhaul} sem fio
em \textit{Field Area Networks} de infraestruturas críticas. Sua arquitetura DODAG, aliada
ao algoritmo \textit{Trickle} e às funções objetivo configuráveis, resolve os desafios de
...
Como continuidade desta revisão, trabalhos futuros no laboratório de \textit{backhaul} sem
fio do projeto CISEI-COPEL preveem a avaliação experimental do RPL via simulação
em Contiki-NG/Cooja, em cenários representativos de redes FAN para automação de
distribuição elétrica, com foco em métricas de convergência, resiliência a falhas
de enlace e adequação ao perfil Wi-SUN em topologias de média escala.
